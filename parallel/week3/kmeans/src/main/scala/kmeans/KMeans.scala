...
class KMeans {
  ...
  def fillEmpty(mean: Point) {

  }

  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointGroups = points.par.groupBy( findClosest(_, means) )
    means.par.map( m => if (pointGroups.contains(m)) m -> pointGroups(m)
                        else m -> GenSeq() ).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
      val updated = oldMeans.par.map( m => findAverage(m, classified(m)) )
      updated
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val updated = update(grouped, means)
