...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.foreach {
      case(x, 0) => output(0) = 0
      case(x, idx) => output(idx) = max(x/idx, output(idx - 1))
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    def calc(idx: Int, until: Int, prevMax: Float): Float = {
      if( idx == until ) prevMax
      else calc(idx + 1, until, max(prevMax, input(idx)/idx) )
    }
    calc(from, until, 0f)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    startingAngle: Float, from: Int, until: Int): Unit = {
      ...
      def calc(prevMax: Float, idx: Int, until: Int): Unit = {
        if ( idx < until ) {
          output(idx) = max(prevMax, input(idx)/idx)
          calc(output(idx), idx + 1, until)
        }
      }
      calc(startingAngle, from, until)
  }
