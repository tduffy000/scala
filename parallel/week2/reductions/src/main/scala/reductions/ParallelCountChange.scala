...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if ( money <= 0 || coins.length <= 1 || threshold(money, coins) ) countChange(money, coins)
    else {
  ...
  def moneyThreshold(startingMoney: Int): Threshold =
    (c: Int, coins: List[Int]) => c <= (2 * startingMoney) / 3

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold =
    (c: Int, coins: List[Int]) => coins.length <= (2 * totalCoins) / 3

  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    (c: Int, coins: List[Int]) => c * coins.length <= (startingMoney * allCoins.size) / 2
  }
}
